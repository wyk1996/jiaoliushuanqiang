...
uint8_t checkYKConline = 0;  //用请求费率的方式来检测YKC是否在线。  0= 初始化   1=可以在空闲状态下正常的请求   2=发送过第一次了   3=09请求后，0A收到
...
extern _dlt645_info dlt645_info[GUN_MAX];
...
_4G_SEND_TABLE YKCSendTable[YKC_SEND_FRAME_LEN] = {
    ...
    {0,    0,    CM_TIME_15_SEC, 	YKC_SendPriodSJDataGunBCmd13		},  //发送B卡枪实时数据   上送充电枪实时数据，周期上送时，待机 5 分钟、充电 15 秒
...
_4G_RECV_TABLE YKCRecvTable[YKC_RECV_FRAME_LEN] = {
    ...
    {YKC_RECV_CMD_04			,	YKC_RecvHeartAck		}, 		//心跳返回确认信息
...
uint8_t   YKC_SendRegisterCmd1(void)
{
    ...
    //YKCSendCmd1.GunNum = DisSysConfigInfo.GunNum;  //0x01单枪  0x02双枪
    YKCSendCmd1.GunNum = 0x02;  //0x01单枪  0x02双枪
    YKCSendCmd1.Vsion = 0x10;		//通信版本  V1.6
    YKCSendCmd1.NetState = 0; 		//SIM
    YKCSendCmd1.Operator = 0;		//移动
    memcpy(&YKCSendCmd1.ProVersions,FIRMWARE_VERSION,sizeof(YKCSendCmd1.ProVersions));  //程序版本号 复制到屏幕上面的版本号
...
uint8_t   YKC_SendHearGunACmd3(void)
{
    ...
    if(stChTcb[GUN_A].ucState != CHARGER_FAULT)
    {
...
uint8_t   YKC_SendHearGunBCmd3(void)
{
    ...
    if(stChTcb[GUN_B].ucState != CHARGER_FAULT)
    {
...
uint8_t   YKC_SendSJDataGunACmd13(void)
{
    ...
    if(stChTcb[GUN_A].stGunStat.ucCpStat != CP_12V)
    {
    ...
    if(stChTcb[GUN_A].ucState == CHARGING)
    {
        ...
        {
            YKCSendCmd13[GUN_A].OutVol = stChTcb[GUN_A].stRn8209.usVolt;			//电压		//输出电压  0.1
            YKCSendCmd13[GUN_A].OutCur = stChTcb[GUN_A].stRn8209.usCurrent/10;		//输出电流	0.1
        }
        else
        {
            YKCSendCmd13[GUN_A].OutVol = dlt645_info[GUN_A].out_vol;			//电压		//输出电压  0.1
            YKCSendCmd13[GUN_A].OutCur = stChTcb[GUN_A].stRn8209.usCurrent/10;		//输出电流	0.1
        }
        YKCSendCmd13[GUN_A].ChargeTime = (time(RT_NULL) - stChTcb[GUN_A].stCh.timestart) /60;	//充电时间  min
        YKCSendCmd13[GUN_A].ChargeKwh =  stChTcb[GUN_A].stCh.uiChargeEnergy * 10;	//充电读数   精确到小数点4位
        YKCSendCmd13[GUN_A].ChargeMoney =stChTcb[GUN_A].stCh.uiAllEnergy;	//已充金额	精确到小数点4位
    }
    if(stChTcb[GUN_A].stIOFault.ucFrist == INPUT_YX3_EM)
    {
...
uint8_t   YKC_SendSJDataGunBCmd13(void)
{
    ...
    if(stChTcb[GUN_B].stGunStat.ucCpStat != CP_12V)
    {
        YKCSendCmd13[GUN_B].GunState = 1;    //插枪
    }
    else
    {
    ...
    if(stChTcb[GUN_B].ucState == CHARGING)
    {
        ...
        if(DisSysConfigInfo.energymeter == 1)
        {
            YKCSendCmd13[GUN_B].OutVol = stChTcb[GUN_B].stRn8209.usVolt;			//电压		//输出电压  0.1
            YKCSendCmd13[GUN_B].OutCur = stChTcb[GUN_B].stRn8209.usCurrent/10;		//输出电流	0.1
        }
        else
        {
            YKCSendCmd13[GUN_B].OutVol = dlt645_info[GUN_B].out_vol;			//电压		//输出电压  0.1
            YKCSendCmd13[GUN_B].OutCur = stChTcb[GUN_B].stRn8209.usCurrent/10;		//输出电流	0.1
        }
        YKCSendCmd13[GUN_B].ChargeTime = (time(RT_NULL) - stChTcb[GUN_B].stCh.timestart) /60;	//充电时间  min
        YKCSendCmd13[GUN_B].ChargeKwh =  stChTcb[GUN_B].stCh.uiChargeEnergy * 10;	//充电读数   精确到小数点4位
        YKCSendCmd13[GUN_B].ChargeMoney =stChTcb[GUN_B].stCh.uiAllEnergy;	//已充金额	精确到小数点4位
    }
    if(stChTcb[GUN_B].stIOFault.ucFrist == INPUT_YX3_EM)
    {
...
uint8_t   YKC_SendPriodSJDataGunACmd13(void)
{
    ...
    if(stChTcb[GUN_A].ucState != CHARGING)  //未再充电中5分钟发送一次
    {
        //枪状态改变需要立马发送
...
//            YKCSendCmd13[GUN_A].OutVol = dlt645_info[GUN_A].out_vol;			//电压		//输出电压  0.1
//            YKCSendCmd13[GUN_A].OutCur = stChTcb[GUN_A].stRn8209.usCurrent/10;		//输出电流	0.1
            YKCSendCmd13[GUN_A].OutVol = dlt645_info[GUN_A].out_vol*10;			//电压		//输出电压  0.1
            YKCSendCmd13[GUN_A].OutCur = dlt645_info[GUN_A].out_cur*10;		//输出电流	0.1
        }
        YKCSendCmd13[GUN_A].ChargeTime =(time(RT_NULL) - stChTcb[GUN_A].stCh.timestart) /60;	//充电时间  min
        YKCSendCmd13[GUN_A].ChargeKwh =  stChTcb[GUN_A].stCh.uiChargeEnergy * 10;	//充电读数   精确到小数点4位
        YKCSendCmd13[GUN_A].ChargeMoney =stChTcb[GUN_A].stCh.uiAllEnergy;	//已充金额	精确到小数点4位
...
uint8_t   YKC_SendPriodSJDataGunBCmd13(void)
{
    ...
    if(stChTcb[GUN_B].ucState != CHARGING)  //未再充电中5分钟发送一次
    {
        //枪状态改变需要立马发送
        if(++count < CM_TIME_5_MIN/CM_TIME_15_SEC)
        {
...
//            YKCSendCmd13[GUN_B].OutVol = dlt645_info[GUN_B].out_vol;			//电压		//输出电压  0.1
////            YKCSendCmd13[GUN_B].OutCur = stChTcb[GUN_B].stRn8209.usCurrent/10;		//输出电流	0.1
//			YKCSendCmd13[GUN_B].OutCur = dlt645_info[GUN_B].out_cur/10;		//输出电流	0.1

            YKCSendCmd13[GUN_B].OutVol = dlt645_info[GUN_B].out_vol*10;			//电压		//输出电压  0.1
            YKCSendCmd13[GUN_B].OutCur = dlt645_info[GUN_B].out_cur*10;		//输出电流	0.1
        }
        YKCSendCmd13[GUN_B].ChargeTime =(time(RT_NULL) - stChTcb[GUN_B].stCh.timestart) /60;	//充电时间  min
        YKCSendCmd13[GUN_B].ChargeKwh =  stChTcb[GUN_B].stCh.uiChargeEnergy * 10;	//充电读数   精确到小数点4位
...
uint8_t   YKC_SendStartAck(_GUN_NUM gun)
{
    ...
    if(stChTcb[gun].stGunStat.ucCpStat != CP_12V)
    {
...
uint8_t   PreYKCBill(_GUN_NUM gun,uint8_t *pdata)
{
    ...
    _YKC_SEND_CMD3B * pbill = (_YKC_SEND_CMD3B *)pdata;   //相互赋值，先赋值到指针Pbill  里面的值改变后，传入的值也会被改变。
    ...
    pbill->GunNum = gun+1;
    pbill->StartTime.Second = stChTcb[gun].stCh.uiChStartTime.ucSec * 1000;
    pbill->StartTime.Day =stChTcb[gun].stCh.uiChStartTime.ucDay;
    pbill->StartTime.Hour = stChTcb[gun].stCh.uiChStartTime.ucHour;
    pbill->StartTime.Minute = stChTcb[gun].stCh.uiChStartTime.ucMin;
    pbill->StartTime.Month = stChTcb[gun].stCh.uiChStartTime.ucMonth;
    pbill->StartTime.Year = stChTcb[gun].stCh.uiChStartTime.ucYear - 100;
    ...
    for(i = 0; i <4; i++)
    {
        ...
        pbill->BillFsInfo[i].PowerKwh =stChTcb[gun].stCh.jfpg_kwh[i]*10;
        pbill->BillFsInfo[i].Money = stChTcb[gun].stCh.uiaChargeEnergy[i];
    }
    if(DisSysConfigInfo.energymeter == 1)
    {
        pbill->ChargeStartKwh = (stChTcb[gun].stRn8209.uiE - stChTcb[gun].stCh.uiChargeEnergy) * 10; //充电前电表读数
        pbill->ChargeStopKwh =stChTcb[gun].stRn8209.uiE * 10;
    }
    else
    {
        pbill->ChargeStartKwh = ((dlt645_info[gun].cur_hwh * 1000) - stChTcb[gun].stCh.uiChargeEnergy )* 10;
        pbill->ChargeStartKwh = dlt645_info[gun].cur_hwh* 10000;
    }



    pbill->ChargeAllKwh = stChTcb[gun].stCh.uiChargeEnergy*10;   //小数点4位

    pbill->ChargeMoney = stChTcb[gun].stCh.uiAllEnergy;
    ...
    else
    {
        ...
        //其他原因
        if((stChTcb[gun].stCh.reason == NORMAL) || (stChTcb[gun].stCh.reason == END_CONDITION) || \
                (stChTcb[gun].stCh.reason == NO_CURRENT) ||(stChTcb[gun].stCh.reason == NO_CURRENT_9V) ||( stChTcb[gun].stCh.reason == STOP_CHARGEFULL) )
        {
        ...
        else if(stChTcb[gun].stCh.reason == UNPLUG)
        {
...
uint8_t YKC_SendBill(_GUN_NUM gun)
{
    ...
    YKCSendCmd3B[gun].StartTime.Second = stChTcb[gun].stCh.uiChStartTime.ucSec * 1000;
    YKCSendCmd3B[gun].StartTime.Day =stChTcb[gun].stCh.uiChStartTime.ucDay;
    YKCSendCmd3B[gun].StartTime.Hour = stChTcb[gun].stCh.uiChStartTime.ucHour;
    YKCSendCmd3B[gun].StartTime.Minute = stChTcb[gun].stCh.uiChStartTime.ucMin;
    YKCSendCmd3B[gun].StartTime.Month = stChTcb[gun].stCh.uiChStartTime.ucMonth;
    YKCSendCmd3B[gun].StartTime.Year = stChTcb[gun].stCh.uiChStartTime.ucYear - 100;
    ...
    for(i = 0; i <4; i++)
    {
        ...
        YKCSendCmd3B[gun].BillFsInfo[i].PowerKwh =stChTcb[gun].stCh.jfpg_kwh[i]*10;
        YKCSendCmd3B[gun].BillFsInfo[i].Money = stChTcb[gun].stCh.uiaChargeEnergy[i];
    }
    if(DisSysConfigInfo.energymeter == 1)
    {
        YKCSendCmd3B[gun].ChargeStartKwh = (stChTcb[gun].stRn8209.uiE - stChTcb[gun].stCh.uiChargeEnergy) * 10; //充电前电表读数
        YKCSendCmd3B[gun].ChargeStopKwh =stChTcb[gun].stRn8209.uiE * 10;
    }
    else
    {
        YKCSendCmd3B[gun].ChargeStartKwh = ((dlt645_info[gun].cur_hwh * 1000) - stChTcb[gun].stCh.uiChargeEnergy )* 10;
        YKCSendCmd3B[gun].ChargeStartKwh = dlt645_info[gun].cur_hwh* 10000;
    }



    YKCSendCmd3B[gun].ChargeAllKwh = stChTcb[gun].stCh.uiChargeEnergy*10;   //小数点4位

    YKCSendCmd3B[gun].ChargeMoney = stChTcb[gun].stCh.uiAllEnergy;
    ...
    else
    {
        ...
        else if(stChTcb[gun].stCh.reason == E_END_APP_BALANC)
        {
...
{
    if((stChTcb[gun].stCh.uiAllEnergy + 100) >= Balance[gun]*100) //充电金额 > 余额时，停止
    {
        return 1;   //App停止
    }
    return 0;
...
/***************************************************************
**Function   :_YKC_RestUpdataData
**Description:YKC断电/宕机复位 充电信息续充
**Input      :gun: [输入/出]
**Output     :
**Return     :
**note(s)    :
**Author     :CSH
**Create_Time:2023年8月14日09:06:54
***************************************************************/
uint8_t _YKC_RestUpdataData(_GUN_NUM gun)
{
    _YKC_RECV_CMD34 *pBill	= (_YKC_RECV_CMD34 *)APP_GetBillInfo(gun);  //赋值
    memcpy(YKCRecvCmd34[gun].SerialNum,pBill->SerialNum,16);	//订单号
    return TRUE;
}
...
uint8_t   YKC_RecvRegisterAck(uint8_t *pdata,uint16_t len)
{
    ...
    if(pdata[7] == 0x00) //0：注册不通过   0x01登录失败
    {
        ...
        {
            if(stChTcb[gun].ucState != CHARGING)   //没在充电中读取
            {
                ReadFmBill(gun);   //是否需要发送订单
            }
        }
        //发送计费模型验证
        mq_service_send_to_4gsend(APP_RATE_MODE,GUN_A ,0 ,NULL);

        //发送实时数据 A和B都要发送，这样联网速度快
        mq_service_send_to_4gsend(APP_SJDATA_QUERY,GUN_A ,0 ,NULL);
        mq_service_send_to_4gsend(APP_SJDATA_QUERY,GUN_B ,0 ,NULL);
    }
...
static uint8_t   YKC_RecvPricAck(uint8_t *pdata,uint16_t len)
{
    ...
    //rt_kprintf("shoudao-0x090A\r\n");
    checkYKConline =3; //双枪都空闲时，间隔1个小时（主要是判断是不是离线了），09请求费率，如果收到这个标志位，就证明已经离线
    ...
    for(gun = 0; gun <= DisSysConfigInfo.GunNum; gun++)
    {
        if(stChTcb[gun].ucState != CHARGING)
        {
            ...
            memcpy(&YKCGunBill[gun],&YKCRecvCmd0A,sizeof(YKCRecvCmd0A));
        }
...
static uint8_t  YKC_RecvCard(uint8_t *pdata,uint16_t len)
{
    ...
    {
        send_ch_ctl_msg(gun,1,0,4,0);          //直接开始充电充满
...
//====YKC在长连接的情况下，双枪都空闲1小时的情况一下，请求费率一下
uint8_t  APP_YKCSENDFEILV(uint32_t timeout)
{
    static uint8_t QUENUM = 0; //请求次数
    static uint32_t  current_time = 0,last_time = 0;    //连续空闲1个小时
    ...
    if((stChTcb[GUN_A].ucState == STANDBY) && (stChTcb[GUN_B].ucState == STANDBY))
    {
        current_time = rt_tick_get();  	//判断是否是连续空闲
        //rt_kprintf("kongxian++ %d\r\n",checkYKConline);
    }
    else
    {
    ...
    if(APP_GetAppRegisterState(LINK_NUM) == STATE_OK)  //注册成功后 联网的状态下
    {
        ...
        else if((((current_time >= last_time) ? (current_time - last_time):((0xFFFFFFFF - last_time) + current_time)) > CH_TIME_15_SEC)&&(checkYKConline ==2))
        {
            ...
            QUENUM++;
            rt_kprintf("qingqiucishu: %d \r\n",QUENUM);
            if(QUENUM > 6)
            {
                APP_SetNetNotConect(0);		//关闭网络操作
                checkYKConline = 1;
                rt_kprintf("close-net\r\n");
            }
        }
        else if(checkYKConline ==3)   //0A收到，正常返回
        {
            rt_kprintf("shoudao-zhengchang\r\n");
            current_time = rt_tick_get();   //时间和标志位，全部都清空时
            last_time = rt_tick_get();
            checkYKConline = 1;
        }
    }
