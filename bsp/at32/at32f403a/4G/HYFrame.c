    ...
    for(i = 0; i < HY_SEND_FRAME_LEN; i++)
    {
                ...
                ((HYSendTable[i].curtime + (0xFFFFFFFFu - HYSendTable[i].lasttime)) >= HYSendTable[i].cycletime))
        {
            ...
            if(HYSendTable[i].Sendfunc != NULL)
            {
                ...
                //rt_thread_mdelay(100);  //主要是后台没有处理粘包，导致心跳收不到，处理后，就可以删除了
            }
        }
    }
    return RT_TRUE;
...
//    }
    ...
    memcpy(SendHYCmd1102.version,FIRMWARE_VERSION,sizeof(SendHYCmd1102.version));
...
static uint8_t   HY_SendRegister(void)
{
    ...
    SendHYCmd106.gun_index = 2;					//充电桩枪数量
...
uint8_t   HY_SendDevStateA(void)
{
    ...
    else if(stChTcb[GUN_A].ucState == WAIT_STOP)
    {
        SendHYCmd104[GUN_A].work_stat = 7;
    }
    else if(stChTcb[GUN_A].ucState == INSERT)
    {
    ...
    {
        SendHYCmd104[GUN_A].cumulative_charge_fee = stChTcb[GUN_A].stCh.uiAllEnergy/100;   			//充电电费


        if(DisSysConfigInfo.energymeter == 1)
        {
            SendHYCmd104[GUN_A].now_meter_kwh_num = stChTcb[GUN_A].stRn8209.uiE / 10;					//当前充电读表
            SendHYCmd104[GUN_A].ac_a_vol = stChTcb[GUN_A].stRn8209.usVolt;			//电压
            SendHYCmd104[GUN_A].ac_a_cur  =  stChTcb[GUN_A].stRn8209.usCurrent/10;	//电流
        }
        else
        {
            SendHYCmd104[GUN_A].now_meter_kwh_num = dlt645_info[GUN_A].cur_hwh * 100;					//当前充电读表
            SendHYCmd104[GUN_A].ac_a_vol = dlt645_info[GUN_A].out_vol*10;			//电压
            SendHYCmd104[GUN_A].ac_a_cur  =  dlt645_info[GUN_A].out_cur*10;	//电流
        }
        SendHYCmd104[GUN_A].bms_charge_mode = 0;										//自动充满
        SendHYCmd104[GUN_A].cum_charge_money_amount =  stChTcb[GUN_A].stCh.uiAllEnergy/100;		//累计充电金额
        SendHYCmd104[GUN_A].cum_charge_kwh_amount = stChTcb[GUN_A].stCh.uiChargeEnergy / 10;			//累计充电电量
        ...
        SendHYCmd104[GUN_A].charged_sec = time(RT_NULL) - stChTcb[GUN_A].stCh.timestart;//充电时间 秒
    }
    else
    {
        ;
...
uint8_t   HY_SendDevStateB(void)
{
    ...
    //SendHYCmd104[GUN_A].gun_cnt = DisSysConfigInfo.GunNum;					//充电枪数量
    SendHYCmd104[GUN_B].gun_cnt = 0x01;
    ...
    else if(stChTcb[GUN_B].ucState == WAIT_STOP)
    {
        SendHYCmd104[GUN_B].work_stat = 7;
    }
    else if(stChTcb[GUN_B].ucState == INSERT)
    {
        SendHYCmd104[GUN_B].work_stat = 1;
    }
    else
    {
    ...
    {
        SendHYCmd104[GUN_B].cumulative_charge_fee = stChTcb[GUN_B].stCh.uiAllEnergy/100;   			//充电电费


        if(DisSysConfigInfo.energymeter == 1)
        {
            SendHYCmd104[GUN_B].now_meter_kwh_num = stChTcb[GUN_B].stRn8209.uiE / 10;					//当前充电读表
            SendHYCmd104[GUN_B].ac_a_vol = stChTcb[GUN_B].stRn8209.usVolt;			//电压
            SendHYCmd104[GUN_B].ac_a_cur  =  stChTcb[GUN_B].stRn8209.usCurrent/10;	//电流
        }
        else
        {
            SendHYCmd104[GUN_B].now_meter_kwh_num = dlt645_info[GUN_B].cur_hwh * 100;					//当前充电读表
            SendHYCmd104[GUN_B].ac_a_vol = dlt645_info[GUN_B].out_vol*10;			//电压
            SendHYCmd104[GUN_B].ac_a_cur  =  dlt645_info[GUN_B].out_cur*10;	//电流
        }
        SendHYCmd104[GUN_B].bms_charge_mode = 0;										//自动充满
        SendHYCmd104[GUN_B].cum_charge_money_amount =  stChTcb[GUN_B].stCh.uiAllEnergy/100;		//累计充电金额
        SendHYCmd104[GUN_B].cum_charge_kwh_amount = stChTcb[GUN_B].stCh.uiChargeEnergy / 10;			//累计充电电量
        ...
        SendHYCmd104[GUN_B].charged_sec = time(RT_NULL) - stChTcb[GUN_B].stCh.timestart;//充电时间 秒
    }
    else
    {
        ;
...
//		NORMAL
    ...
    pbill->charge_start_time[1] = HEXtoBCD(stChTcb[gun].stCh.uiChStartTime.ucYear - 100);
    pbill->charge_start_time[2] = HEXtoBCD(stChTcb[gun].stCh.uiChStartTime.ucMonth);
    pbill->charge_start_time[3] = HEXtoBCD(stChTcb[gun].stCh.uiChStartTime.ucDay);
    pbill->charge_start_time[4] = HEXtoBCD(stChTcb[gun].stCh.uiChStartTime.ucHour);
    pbill->charge_start_time[5] = HEXtoBCD(stChTcb[gun].stCh.uiChStartTime.ucMin);
    pbill->charge_start_time[6] = HEXtoBCD(stChTcb[gun].stCh.uiChStartTime.ucSec);
    if(stChTcb[gun].ucState == CHARGING)
    {
        pbill->charge_time = time(RT_NULL) - stChTcb[gun].stCh.timestart;			//充电时间长度  单位秒
    }
    else
    {
        pbill->charge_time = stChTcb[gun].stCh.timestop - stChTcb[gun].stCh.timestart;			//充电时间长度  单位秒
    }

    //结束时间
    pbill->charge_end_time[0] = 0x20;
    pbill->charge_end_time[1] = HEXtoBCD(stChTcb[gun].stCh.uiChStoptTime.ucYear - 100);
    pbill->charge_end_time[2] = HEXtoBCD(stChTcb[gun].stCh.uiChStoptTime.ucMonth);
    pbill->charge_end_time[3] = HEXtoBCD(stChTcb[gun].stCh.uiChStoptTime.ucDay);
    pbill->charge_end_time[4] = HEXtoBCD(stChTcb[gun].stCh.uiChStoptTime.ucHour);
    pbill->charge_end_time[5] = HEXtoBCD(stChTcb[gun].stCh.uiChStoptTime.ucMin);
    pbill->charge_end_time[6] = HEXtoBCD(stChTcb[gun].stCh.uiChStoptTime.ucSec);



    pbill->charge_kwh_amount  = stChTcb[gun].stCh.uiChargeEnergy / 10;
    pbill->total_charge_fee = stChTcb[gun].stCh.uiAllEnergy/100;			//本次充电金额
    pbill->total_service_fee = (stChTcb[gun].stCh.uifwChargeEnergy[0] + stChTcb[gun].stCh.uifwChargeEnergy[1]  \
                                + stChTcb[gun].stCh.uifwChargeEnergy[2] + stChTcb[gun].stCh.uifwChargeEnergy[3])/100;			//本次充电总服务费
...
//		NORMAL
        //其他原因
        if((stChTcb[gun].stCh.reason == NORMAL) || (stChTcb[gun].stCh.reason == END_CONDITION) || (stChTcb[gun].stCh.reason == NO_CURRENT)|| (stChTcb[gun].stCh.reason == NO_CURRENT_9V) )
        {
            pbill->err_no[0] = 0x31;
            pbill->err_no[1] = 0x30;
            pbill->err_no[2] = 0x30;
    ...
    SendHYCmd104[gun].cumulative_charge_fee = stChTcb[gun].stCh.uiAllEnergy/100;   			//充电电费
    if(DisSysConfigInfo.energymeter == 1)
    {
        SendHYCmd104[gun].dc_charge_voltage = stChTcb[gun].stRn8209.usVolt;			//电压
        SendHYCmd104[gun].dc_charge_current  =  stChTcb[gun].stRn8209.usVolt/10;	//电流
    }
    else
    {
        SendHYCmd104[gun].dc_charge_voltage = dlt645_info[gun].out_vol*10;			//电压
        SendHYCmd104[gun].dc_charge_current  =  dlt645_info[gun].out_cur*10;	//电流
    }
    SendHYCmd104[gun].bms_charge_mode = 0;										//自动充满
    SendHYCmd104[gun].cum_charge_money_amount =  stChTcb[gun].stCh.uiAllEnergy/100;		//累计充电金额

    if(DisSysConfigInfo.energymeter == 1)
    {
        SendHYCmd104[gun].now_meter_kwh_num = stChTcb[gun].stRn8209.uiE / 10;					//当前充电读表
    }
    else
    {
        SendHYCmd104[gun].now_meter_kwh_num = dlt645_info[gun].cur_hwh * 100;					//当前充电读表
    }
...
static uint8_t   HY_RecvStartCharge(uint8_t *pdata,uint16_t len)
{
    ...
    if(pcmd7->gun_index == 0)
    {
        ...
        send_ch_ctl_msg(GUN_A,1,0,4,0);
    }
    else if(pcmd7->gun_index == 1)
    {
        memcpy(&RecvHYCmd7[GUN_B],pcmd7,sizeof(hy_cmd_7));
        APP_SetStartNetState(GUN_B,NET_STATE_ONLINE);		//设置为在线充电
        mq_service_send_to_4gsend(APP_START_ACK,GUN_B ,0 ,NULL);
        send_ch_ctl_msg(GUN_B,1,0,4,0);
    }
    DispControl.CurSysState = DIP_STATE_NORMAL;  //自动恢复到正常的界面(主要为IC卡配置)
    return RT_TRUE;
}
...
static uint8_t   HY_RecvStopCharge(uint8_t *pdata,uint16_t len)
{
    ...
    if(pcmd11->gun_index == 0)
    {
        ...
        send_ch_ctl_msg(GUN_A,2,0,4,0);
    }
    else if(pcmd11->gun_index == 1)
    {
        mq_service_send_to_4gsend(APP_STOP_ACK,GUN_B ,0 ,NULL);
        send_ch_ctl_msg(GUN_B,2,0,4,0);
    }
    else
    {
        return RT_FALSE;
...
static uint8_t   HY_RecvHearAck(uint8_t *pdata,uint16_t len)
{
    ...
    if(((IfSendUpdataAck) && (stChTcb[GUN_A].ucState == STANDBY) && (stChTcb[GUN_B].ucState == STANDBY)) || \
            ((IfSendUpdataAck) && (stChTcb[GUN_A].ucState == CHARGER_FAULT)))   //空闲和故障才可以升级
    {
...
static uint8_t   HY_RecvRegisterAck(uint8_t *pdata,uint16_t len)
{
    ...
    APP_SetAppRegisterState(LINK_NUM,STATE_OK); //注册成功
...
static uint8_t   HY_RecvCardStart(uint8_t *pdata,uint16_t len)
{
    hy_cmd_33 *pcmd33 = (hy_cmd_33 *)pdata;
    if(pdata == NULL)
    {
        return RT_FALSE;
    }
    ...
    {
        _4G_SetStartType(pcmd33->gun_index,_4G_APP_CARD);		//设置卡启动
    }
    return RT_TRUE;
}
...
* Return       :
* Note(s)      :
* Contributor  : 2023-7-28 16:53:19
*****************************************************************************/
//static uint8_t   HY_RecvUpdata(uint8_t *pdata,uint16_t len)
//{
//    int length;
