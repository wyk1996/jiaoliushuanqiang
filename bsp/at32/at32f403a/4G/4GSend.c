            ...
            case APP_STOP_BILL:		//停止结算
                ...
                if(stMsg.uiMsgVar < GUN_MAX) //枪号保护
                {
...
#else
                    {
                        ...
                        WriterFmBill(stMsg.uiMsgVar,1);  //写入离线记录
                        APP_SetResendBillState(stMsg.uiMsgVar,RT_TRUE);  //设置需要重发
                        _4G_SendBill((_GUN_NUM)stMsg.uiMsgVar);  //停止后，发送订单
        ...
        for(i = 0; i < GUN_MAX; i++)
        {
            if(APP_GetResendBillState(i) == RT_TRUE)
            {
                if((APP_GetSIM7600Status() == STATE_OK) && (APP_GetModuleConnectState(0) == STATE_OK) && (APP_GetAppRegisterState(0) == STATE_OK)	) //连接上服务器
                {
                    if(i == GUN_A)
                    {
                        ReSendBill(i,pBillGunA,0);  //连上网后，重发离线订单
                    }
                    else
                    {
                        //	WriterFmBill(1,1);
