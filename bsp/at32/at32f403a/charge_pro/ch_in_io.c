...
static uint8_t is_set_input_io_fault(uint8_t ucIndex,int iData)
{
    ...
    if(0 == stFault.stInputIo[ucIndex].ucFault)
    {
        ...
        /* 故障发生 */
        stChTcb[GUN_A].stIOFault.uiFaultFlg |= 0x00000001 << ucIndex;
        stChTcb[GUN_B].stIOFault.uiFaultFlg |= 0x00000001 << ucIndex;
        if(stChTcb[GUN_A].stIOFault.ucFrist == 0)
        {
            stChTcb[GUN_A].stIOFault.ucFrist = ucIndex;
            stChTcb[GUN_B].stIOFault.ucFrist = ucIndex;
        }
    }
...
static uint8_t is_input_io_fault_resume(uint8_t ucIndex,int iData)
{
    ...
    if(1 == stFault.stInputIo[ucIndex].ucFault)
    {
        ...
        stChTcb[GUN_A].stIOFault.uiFaultFlg &= ~(0x00000001 << ucIndex);
        stChTcb[GUN_B].stIOFault.uiFaultFlg &= ~(0x00000001 << ucIndex);
        stChTcb[GUN_A].stIOFault.ucFrist = 0;
        stChTcb[GUN_B].stIOFault.ucFrist = 0;
        for(i = 0; i < INPUT_YX_MAX; i++)
        {
            if(stChTcb[GUN_A].stIOFault.uiFaultFlg & ((0x00000001) << i))
            {
                stChTcb[GUN_A].stIOFault.ucFrist = i;
                stChTcb[GUN_B].stIOFault.ucFrist = i;
            }
        }
    }
    stFault.stInputIo[ucIndex].ucLowCnt = 0;
