...
//=====20221102AT大板子=====断电重启后，可以保存时间，但是时间非常不准(当前已经修改：低速外部晶振，时间比较准，24小时大该相差1-2s)=====
static rt_err_t rt_rtc_config(void)
{
    ...
    crm_rtc_clock_select(CRM_RTC_CLOCK_LICK);  //低速内部晶振
#else
    crm_rtc_clock_select(CRM_RTC_CLOCK_LEXT); //低速外部晶振
...
////=====20221102AT大板子=====下面这个时间设置后，断电重启后不保存，但是时间走的准=======
//static rt_err_t rt_rtc_config(void)
//{
